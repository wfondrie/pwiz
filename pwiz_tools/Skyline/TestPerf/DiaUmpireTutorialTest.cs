...
namespace TestPerf
{
    ...
    public class DiaUmpireTutorialTest : AbstractFunctionalTestEx
    {
        ...
        private class AnalysisValues
        {
            ...
            public string FastaPathForSearch => "napedro_3mixed_human_yeast_ecoli_20140403_iRT.fasta";
            public string FastaPath =>
                IsWholeProteome
                    ? "napedro_3mixed_human_yeast_ecoli_20140403_iRT.fasta"
        ...
        public void TestDiaTtofDiaUmpireTutorial()
        {
            ...
            _analysisValues = new AnalysisValues
            {
                ...
                ScoringModelCoefficients = "0.0872|-0.4325|4.6642|-0.0718|-0.5895|0.9069|0.1454|-0.1057",
                MassErrorStats = new[]
                {
                    new[] {3.2, 3.8},
                    new[] {3.1, 3.1},
        ...
        public void TestDiaTtofDiaUmpireFullTutorial()
        {
            _analysisValues = new AnalysisValues
            {
                ...
                ScoringModelCoefficients = "0.2110|-0.6707|3.7894|0.1967|-0.1875|0.6021|0.0609|-0.0468",
                MassErrorStats = new[]
                {
                    new[] {2.6, 5},
                    new[] {2.5, 4.6},
        ...
        public void TestDiaQeDiaUmpireTutorial()
        {
            ...
            _analysisValues = new AnalysisValues
            {
                ...
                LibraryPeptideCount = 9807,
                IrtSlope = 2.605,
                IrtIntercept = -45.946,
                TargetCounts = new[] {13, 162, 194, 1164},
                FinalTargetCounts = new[] {9, 162, 194, 1164},
                ScoringModelCoefficients = "0.3838|-0.9471|4.6887|-0.6233|-0.0920|0.8417|0.1163|-0.0008",
                MassErrorStats = new[]
                {
                    new[] {1.9, 3},
                    new[] {1.4, 2.9},
        ...
        public void TestDiaQeDiaUmpireFullTutorial()
        {
            _analysisValues = new AnalysisValues
            {
                ...
                MinPeptidesPerProtein = 2,
                RemoveDuplicates = true,
                LibraryPeptideCount = 16489,
                IrtSlope = 2.602,
                IrtIntercept = -45.821,
                ChromatogramClickPoint = new PointF(18.19f, 1.8e6f),
                TargetCounts = new[] { 4065, 32056, 34984, 209904 }, // From MSAmanda searching DDA files: { 6437, 21618, 22195, 133170 }, from Comet: { 4065, 32056, 34984, 209904 }
                FinalTargetCounts = new[] { 1696, 15718, 17019, 102114 }, // From MSAmanda searching DDA files: { 5610, 17434, 17846, 107076 }, from Comet: { 2081, 22732, 24786, 148716 }
                ScoringModelCoefficients = "0.2207|-0.8154|3.3823|1.5038|-0.0642|0.7447|0.1118|-0.0437",
                MassErrorStats = new[]
                {
                    new[] {1.9, 3.8},
                    new[] {1.4, 3.7},
        ...
        private void SetInstrumentType(InstrumentSpecificValues instrumentValues)
        {
            ...
            TestFilesZipPaths = new[]
            {
                ...
                @"TestPerf\DiaUmpireTutorialTest.zip",
...
//            IsPauseForCoverShot = true;
        ...
        {
            var match = Regex.Match(irtFormula, @"iRT = (?<slope>\d+\.\d+) \* Measured RT (?<sign>[+-]) (?<intercept>\d+\.\d+)");
        ...
        {
            Assert.AreEqual("IrtSlope = 3.005,\r\nIrtIntercept = -67.173,\r\n", ParseIrtProperties("iRT = 3.005 * Measured RT - 67.173"));
            if (IsRecordMode)
                Console.WriteLine();
            ...
            int currentPage = 2;
            PauseForScreenShot<ImportPeptideSearchDlg.SpectraPage>("Import Peptide Search - Build Spectral Library empty page", ++currentPage);
            ...
            string fastaPathForImport = _analysisValues.IsWholeProteome
                ? TestFilesDirs[1].GetTestPath(_analysisValues.FastaPath)
                : GetTestPath(_analysisValues.FastaPath);
            Assert.IsTrue(File.Exists(fastaPathForImport));

            string fastaPathForSearch = TestFilesDirs[1].GetTestPath(_analysisValues.FastaPathForSearch);
            ...
            PauseForScreenShot<ImportPeptideSearchDlg.SpectraPage>("Import Peptide Search - Build Spectral Library populated page", ++currentPage);
            ...
            PauseForScreenShot<ImportPeptideSearchDlg.MatchModsPage>("Import Peptide Search - After adding modifications page", ++currentPage);
